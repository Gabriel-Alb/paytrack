<template>
  <label class="block min-w-0 text-sm">
    ...
    <select aria-label="Empresa" :value="modelValue ?? ''" :required="!filter" :disabled="loading || (!filter && user?.role === 'user' && companies.length === 1)"
      :class="filter
        ? 'h-10 w-full rounded-xl border border-black/[0.10] bg-white px-3 text-[13px] font-medium text-[#52525b] outline-none focus:border-[#166534]/40 focus:shadow-[0_0_0_3px_rgba(22,101,52,0.08)]'
        : 'h-10 w-full rounded-[10px] border border-black/[0.11] bg-[#f8f8f8] px-3 text-sm text-[#202124] outline-none focus:border-[#166534] disabled:opacity-70'"
