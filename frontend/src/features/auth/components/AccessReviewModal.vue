<template>
  <BaseModal :model-value="!!userId" title="Avaliar acesso" @close="$emit('close')">
    ...
    <dl v-if="person" class="space-y-4 text-sm">
      ...
      <div v-if="person.accessStatus==='pending'">
        <dt><label for="access-role" class="text-[#71717a]">Perfil</label></dt>
        <dd class="mt-1"><select id="access-role" v-model="role" :disabled="busy" class="min-h-11 rounded-lg border border-black/15 px-3"><option value="user">Usuário</option><option value="admin">Administrador</option></select></dd>
      </div>
    </dl>
    <template #footer>
...
watch(()=>props.userId,async (id,_,onCleanup)=>{
  person.value=null;error.value='';role.value='user'
  if (!id) return
  let current=true
...
async function decide(action) {
  ...
  try {await request(`/users/${props.userId}/access`,{method:'PATCH',body:action==='approve' ? {action,role:role.value} : {action}});emit('updated');emit('close')}
