    ...
    <dl v-if="person" class="space-y-4 text-sm">
      ...
      <div v-if="['pending','active','blocked'].includes(person.accessStatus)">
        ...
        <dd v-else class="mt-2 max-h-48 space-y-2 overflow-y-auto rounded-lg border border-black/15 p-3">
          ...
          <p v-if="!companyIds.length" class="text-xs text-red-800">Selecione pelo menos uma empresa.</p>
        </dd>
      </div>
    </dl>
    <template #footer>
      <button v-for="action in actions" :key="action.value" :disabled="busy || loading || (['approve','edit'].includes(action.value) && role === 'user' && !companyIds.length)" class="min-h-11 rounded-lg border border-black/15 px-4 py-2 text-sm font-semibold disabled:opacity-50" :class="action.value==='approve' && 'bg-[#166534] text-white'" @click="decide(action.value)">{{ busy ? 'Salvando…' : action.label }}</button>
    </template>
  </BaseModal>
</template>
<script setup>
...
watch(()=>props.userId,async (id,_,onCleanup)=>{
  person.value=null;error.value='';role.value='user';companyIds.value=[]
  if (!id) return
  let current=true
...
async function decide(action) {
  ...
  try {await request(`/users/${props.userId}/access`,{method:'PATCH',body:['approve','edit'].includes(action) ? {action,role:role.value,companyIds:companyIds.value} : {action}});emit('updated');emit('close')}
