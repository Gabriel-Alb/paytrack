<template>
  <section class="rounded-2xl border border-black/[0.07] bg-white p-5 sm:p-7">
    ...
    <ul v-else class="mt-5 divide-y divide-black/5"><li v-for="person in items" :key="person.id" class="flex flex-wrap items-center justify-between gap-3 py-4"><div class="min-w-0"><p class="break-words font-medium">{{ person.name }}</p><p class="break-all text-sm text-[#71717a]">{{ person.email }}</p></div><button class="min-h-11 rounded-lg border border-black/15 px-4 text-sm font-semibold text-[#166534]" @click="selected=person.id">Ver acesso</button></li></ul>
