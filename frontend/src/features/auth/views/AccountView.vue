<template>
  <section class="max-w-xl rounded-2xl border border-black/[0.07] bg-white p-5 sm:p-7">
    ...
    <form class="space-y-5" @submit.prevent="submit">
      ...
      <PasswordField id="new-password" v-model="newPassword" label="Nova senha" hint="Use de 15 a 128 caracteres." />
...
async function submit() {
  ...
  if ([...newPassword.value].length<15 || [...newPassword.value].length>128) {error.value='A senha deve ter entre 15 e 128 caracteres.';return}
  busy.value=true
