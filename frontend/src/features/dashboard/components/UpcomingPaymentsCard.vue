<template>
    <article class="min-w-0 rounded-xl border border-black/[0.08] bg-white p-4 sm:p-5">
        ...
        <div v-if="items.length" class="mt-4 divide-y divide-black/[0.07]">
            <button v-for="(payment, index) in items" :key="payment.id ?? `${payment.name}-${index}`" type="button"
                ...
                <div
                    ...
                    <p class="mt-0.5 truncate text-[8px] text-[#71717a] sm:text-[10px]">
                        {{ payment.date }}
                    </p>
                </div>

                <svg viewBox="0 0 24 24"
                    class="h-4 w-4 shrink-0 text-[#71717a] transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:translate-x-0.5 group-active:translate-x-0"
                    aria-hidden="true">
