    ...
    <BaseInfoCard>
        ...
        <div class="mt-5 grid grid-cols-2 gap-x-4 gap-y-4">
            ...
            <div class="min-w-0">
                ...
                <p class="mt-1 truncate text-[13px] font-medium text-[#3f3f46]">
                    {{ formatPercentage(loan.interest) }}
                </p>
            </div>
        </div>
        ...
        <div class="mt-5 flex items-end justify-between gap-4 border-t border-black/[0.06] pt-4">
            ...
            <p v-if="loan.daysLate > 0" class="shrink-0 text-[11px] font-medium" :class="loan.status === 'overdue'
                    ? 'text-[#b91c1c]'
                    : 'text-[#b45309]'
                ">
                {{ lateDaysLabel }}
...
const statusLabel = computed(() => {
    ...
    return labels[props.loan.status] ?? props.loan.status
})

const statusClasses = computed(() => {
    const classes = {
        'on-time': 'bg-[#166534] text-white',
        attention: 'bg-[#d97706] text-white',
        overdue: 'bg-[#b91c1c] text-white',
    }

    return classes[props.loan.status] ?? 'bg-[#52525b] text-white'
})

const paidInstallmentsLabel = computed(() => {
    const paid = props.loan.paidInstallments ?? 0
    const total = props.loan.installmentCount ?? 0

    return `${paid} de ${total}`
})

const lateDaysLabel = computed(() => {
    const days = props.loan.daysLate ?? 0
