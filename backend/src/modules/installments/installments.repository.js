...
export function findInstallment(id) {
  return database()
    ...
    l.revision FROM installments i JOIN loans l ON l.id=i.loan_id WHERE i.id=?`,
    )
    .get(id);
}
...
export function listInstallments(loanId) {
  return database()
    ...
    FROM installments i LEFT JOIN late_fees f ON f.installment_id=i.id
...
export function reconcileInstallments(date, loanId) {
  database()
    ...
    paid_amount=COALESCE((SELECT SUM(p.amount) FROM payments p WHERE p.installment_id=installments.id AND p.voided_at IS NULL),0),
    paid_at=(SELECT MAX(p.payment_date) FROM payments p WHERE p.installment_id=installments.id AND p.amount>0 AND p.voided_at IS NULL)
    WHERE (@loan IS NULL OR loan_id=@loan) AND loan_id IN (SELECT id FROM loans WHERE status<>'cancelled')`,
    )
...
export function reconcileFees(date, loanId) {
  ...
  database()
    .prepare(
      ...
      days_late=MAX(0, CAST(julianday(COALESCE((SELECT paid_at FROM installments WHERE id=installment_id),@date))
        -julianday((SELECT due_date FROM installments WHERE id=installment_id)) AS INTEGER)),
      paid_amount=COALESCE((SELECT SUM(p.late_fee_amount) FROM payments p WHERE p.installment_id=late_fees.installment_id AND p.voided_at IS NULL),0),
      paid_at=(SELECT MAX(p.payment_date) FROM payments p WHERE p.installment_id=late_fees.installment_id AND p.late_fee_amount>0 AND p.voided_at IS NULL)
    WHERE installment_id IN (SELECT i.id FROM installments i JOIN loans l ON l.id=i.loan_id
      WHERE l.status<>'cancelled' AND (@loan IS NULL OR l.id=@loan))`,
    )
    .run({ date, loan: loanId });
  database()
    .prepare(
      `UPDATE late_fees SET amount=days_late*(SELECT l.late_fee_per_day FROM loans l
    JOIN installments i ON i.loan_id=l.id WHERE i.id=installment_id), updated_at=CURRENT_TIMESTAMP
    WHERE status<>'waived' AND installment_id IN (SELECT id FROM installments WHERE @loan IS NULL OR loan_id=@loan)`,
    )
    .run({ loan: loanId });
  database()
    .prepare(
    ...
    paid_at=CASE WHEN paid_amount>=amount THEN paid_at ELSE NULL END
    WHERE installment_id IN (SELECT id FROM installments WHERE @loan IS NULL OR loan_id=@loan)`,
    )
    .run({ loan: loanId });
}
...
export function loanBalances(loanId, date) {
  return database()
    ...
    FROM loans l LEFT JOIN installments i ON i.loan_id=l.id LEFT JOIN late_fees f ON f.installment_id=i.id
...
export function clientBalances(clientId, attentionDays, date) {
  return database()
    ...
    FROM clients c WHERE @client IS NULL OR c.id=@client`,
    )
